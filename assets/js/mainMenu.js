                ...
                success: function (data) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        html += `
                                ...
                                <td>${data[i].smm_order}</td>
                                <td class="">${data[i].smm_updated_date}</td>
                                <td class="">${data[i].smm_updated_by}</td>
                                <td><button class="btnStatus rounded-3 fw-semibold badge bg-${data[i].smm_status_flg == 1 ? 'success' : 'danger'} me-1" id="flgStatus" data-sa-id="${data[i].smm_id}" value="${data[i].smm_status_flg}">${data[i].smm_status_flg == 1 ? 'Enable' : 'Disable'}</button></td>
                                <td class="">
                                </li>
                                </ul>
                                </div><a href="" class="tblEditBtn btn btn-warning" data-bs-toggle="modal" data-bs-target="#mdlEdit" id="btnEdit" data-id="${data[i].smm_id}">
...
$.ajax({
    ...
    success: (response) => {
        ...
        //     const data = response[i];
            ...
            $('#edtOrderNo').val(response.data.smm_order)
    ...
    $(document).ready(function () {
        $('#btnSaveEdit').on('click', function () {
            ...
            if (data_mmn && data_mmn.smm_name == MainMenuName && data_mmn.smm_icon == MainMenuIcon && data_mmn.smm_order == OrderNo) {
                Swal.fire({
                    icon: 'success',
                    title: 'Not changed!',
